...
public class GetCSSTheme {
	static HashMap<String, HashMap<String, HashSet<String>>> valueSet = new HashMap<String, HashMap<String, HashSet<String>>>();
		...
		static String incomingFile = "\\css.txt";
		static String pattern = "\\}([^\\}]+?)\\{[^\\{]*?(background\\:|color\\:|background\\-color\\:)([^\\{]+?)[;,\\}]";
		static String testS = "}asd{abackground-color:#dde}ass{background:#asdfff}";
		static String test = "}asd{abackground:#dde;.touch ._3qet .mRequestItem .btn+.btn{margin-left:8px}.touch ._3qet .mRequestItem .btnD{background:#fff;border:1px solid";
...
//	        	 System.out.println("3: " + m.group(3));
	        	 addValues(m.group(1), m.group(2).trim(), m.group(3).trim());
	         } while (m.find(m.end()-1)); //go back one line
	      } else {
